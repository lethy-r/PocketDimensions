...
/**
 * Owns entering and leaving a player's own pocket dimension. Shared by the
 * key item and the /pd menu so both paths behave identically.
 */
public class DimensionService {
    ...
    /** Loads or creates the player's dimension. @return null when creation failed or was cancelled. */
    public World ensureDimension(Player player) {
        ...
        if (world == null) {
            world = Bukkit.getWorld(WorldUtils.simpleWorldName(worldPath));
        }
        if (world != null) {
            return world;
        }
        ...
        String preset = normalizePreset(settings.getPreset() != null
                ? settings.getPreset()
                : plugin.getConfig().getString("dimension.default-preset", "FLAT"));
        World.Environment environment = parseEnvironment(settings.getEnvironment() != null
                ? settings.getEnvironment()
                : plugin.getConfig().getString("dimension.default-environment", "NORMAL"));

        PocketDimensionCreateEvent event = new PocketDimensionCreateEvent(owner, player.getName(), worldPath,
                preset, environment.name());
        Bukkit.getPluginManager().callEvent(event);
        if (event.isCancelled()) {
            return null;
        }
        ...
        if (world == null) {
            plugin.getLogger().severe("Failed to create the pocket dimension at " + worldPath);
            return null;
        }
        switch (preset) {
        ...
        plugin.getLogger().info("Created pocket dimension '" + worldPath + "' (preset " + preset
